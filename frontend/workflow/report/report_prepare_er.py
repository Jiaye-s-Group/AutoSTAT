...
def report_prepare(agents, parallel=True, max_workers=4):
    ...
    if toc is None:
        st.error(bt("请先生成目录", "Please generate the outline first."))
        return
    ...
    if last_cache_key == current_cache_key and existing_report is not None:
        ...
        if hasattr(existing_report, 'root') and existing_report.root.children:
            st.success(bt(
                "检测到报告要求与大纲未变更，直接复用现有文档结构进行格式转换。",
                "Report requirements and outline are unchanged. Reusing the existing document structure for format conversion.",
            ))
            return
    ...
    # === 汇总各分析模块的摘要 ===
    agent_abstracts = {}
    with st.spinner(bt("正在汇总各分析模块的结果...", "Summarizing results from each analysis module...")):
    ...
    # === 更新 toc 的 模块选择 列表 ===
    with st.spinner(bt("正在匹配各章节所需的分析模块...", "Matching each report section to the required analysis modules...")):
    ...
    # === 初始化报告结构 ===
    doc = Reportcore()
    doc.add_heading(bt("数据分析报告", "Data Analysis Report"), 0)
    ...
    # 串行或并行
    if not parallel:
        with st.spinner(bt("正在串行生成各章节内容（带上下文）...", "Generating each section sequentially with context...")):
            history_content = ""
        ...
        with st.spinner(bt(
            "正在并行生成各章节内容（{workers}线程）...",
            "Generating sections in parallel ({workers} threads)...",
            workers=max_workers,
        )):
            with ThreadPoolExecutor(max_workers=max_workers) as executor:
                # print(toc_with_choice)
                futures = {
                    submit_with_context(executor, process_section, idx, t, toc_with_choice[idx], ""): idx
